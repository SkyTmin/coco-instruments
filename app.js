...
app.post('/api/backup/run', async (req, res) => {
  ...
  // ЭТА РУЧКА ТОЛЬКО СОБИРАЕТ И РОТИРУЕТ АРХИВ. Она НИЧЕГО не отправляет и
  // НИЧЕГО не диспатчит — и это принципиально, потому что зовут её двое:
  // ночной systemd-таймер и первый шаг самого workflow `backup.yml`
  // (`deploy/backup.sh`). Пока отсюда вызывался `triggerBackup()`, получалась
  // петля: прогон → backup.sh → ручка → новый прогон → backup.sh → … Гасил её
  // только 30-секундный троттл диспатча, поэтому круг обрывался не сразу, и
  // владельцу приходило по пять копий в день вместо одной.
  //
  // Отправитель теперь ровно один — расписание `backup.yml`: раннер GitHub
  // единственный, кто достаёт и VPS, и Telegram. Получатель не передаётся
  // сознательно: сервер до `api.telegram.org` не дотягивается, и попытка
  // отправки каждую ночь писала бы в `lastBackupError` ложный сбой доставки,
  // из-за которого диагностика в «Кассе» всегда выглядела бы красной.
  const result = await runBackup(null);
  // А вот несобравшийся архив — настоящая беда, и о ней надо кричать: пропажа
  // копий на неделю началась именно с тихого сбоя. Диагностика в приложении
  // это и так покажет (`lastBackupError`), но ночной запуск никто не видит.
  const admin = getAdminChatId();
  if (!result.ok && admin && (await outboundWorks())) {
    await sendTelegram(
      admin,
      `⚠️ Ночная копия не собралась: ${escapeHtml(String(result.error || 'причина неизвестна'))}`,
    );
  }
  res.json(result);
});
