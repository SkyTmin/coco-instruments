...
- Заодно скин чинит три места, где база захардкожена мимо палитры: лампы
  (`#ffd9a3` / `#ffb459` у ВСЕХ скинов), inset-тени ленты (коричневые) и
  подсветка выигрыша (золото `#ffb43c` / `#ffc23c`). Чинит только у себя —
  общий код не трогали.
