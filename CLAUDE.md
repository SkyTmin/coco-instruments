...
- **Счётчик — одометр (`components/Odometer`), а не текст.** Текст меняется
  ПОДМЕНОЙ, между «1240» и «1310» нет ни одного кадра. Арифметика колёс — в
  `lib/odometer.ts` и покрыта тестами, потому что ошибка здесь незаметна на
  глаз: колесо показывает половинку цифры, и счётчик читается как сломанный.
  **Окно подхвата у разряда `p` — последняя его 10^-p доля**, а не общее для
  всех: с общим окном на 49 999 006 старший разряд висит на 99,9% пути.
- **Одометр обновляется через ref, а не через состояние.** Счёт идёт каждый
  кадр; гнать его через `setState` — значит перерисовывать полстраницы
  шестьдесят раз в секунду. `runRollup` пишет прямо в `style.transform`.
