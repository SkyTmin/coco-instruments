...
- **Сфера ЗАНИМАЕТ клетку, а не лежит поверх символа.** Символа под ней нет,
  и в восьмёрку эта клетка не идёт — так у Олимпа, и в этом весь размен:
  сфера платит, но загромождает поле и укорачивает цепочку. Именно поэтому
  таблица выплат может быть выше, чем была бы без этого. Проверяется тестом
  «закрытая сферой клетка не попадает ни в один выигрыш».
