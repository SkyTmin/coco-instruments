...
## Лимитированные скины — `src/lib/skin-limits.ts`

- **Лимит — это про то, когда скин можно ЗАБРАТЬ, а не сколько им играть.**
  Взял, пока окно открыто, — остался твоим навсегда (`slotsSkinsOwned` в
  сторе, пишется в `setSlotsPrefs`). Скин, который отбирают обратно по
  календарю, был бы не наградой, а подлостью: игрок выбрал его, привык, и в
  одно утро автомат выглядит иначе.
- **Два вида окна, и оба нужны.** СЕЗОННОЕ (`{from, until}`) возвращается
  каждый год — пропустил, жди год. ДРОП (`{from, until, year}`) разовый и не
  возвращается никогда. Если оставить только один вид, слово
  «лимитированный» перестаёт что-либо значить.
- **Окно умеет переходить через Новый год** (`from` позже `until`) — иначе
  зимний скин закрывался бы 1 января. Это покрыто тестом, не ломайте.
- **Конец окна — это КОНЕЦ дня `until`**, иначе в последний день скин уже
  не дают, хотя на карточке написано, что он ещё доступен.
- Если окно закрылось, а скин не забрали, лист скинов тихо возвращает
  автомат на «Классику»: иначе игра осталась бы с символами, которых игрок
  уже не может выбрать заново.
- Владельцу счётчик дней не показываем — он уже не торопится.
- Сейчас лимитированных два: `pumpkin` (сезонный, 20 сентября — 5 ноября) и
  `abyss` (дроп 2026 года). Символы — из того же Twemoji, что и остальные
  скины; новый скин = каталог в `public/slots/<id>/` на 8 файлов
  (7 символов + `scatter`), запись в `SKINS`, палитра и декор в `theme.css`.
