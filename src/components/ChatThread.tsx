...
import { IconImage, IconPaperclip, IconPencil, IconSend, IconTrash } from '@/components/icons';
...
export function ChatThread({
  ...
  const press = useRef<{ id: string; x: number; y: number; t: number; fired: boolean } | null>(null);
  const longPressTimer = useRef(0);
  const onBubbleDown = (m: NoteMessage, e: ReactPointerEvent) => {
    press.current = { id: m.id, x: e.clientX, y: e.clientY, t: performance.now(), fired: false };
  ...
  return (
    <>
      <div className="chat-scroll">
        {header}
        {!messages.length && (
          <div className="chat-empty">{emptyHint ?? 'Пока пусто. Напишите первое сообщение ниже.'}</div>
        )}
        {messages.map((m) => {
      ...
      <div className="chat-input">
        {editingId && (
          <div className="chat-editing">
            <IconPencil size={15} /> Редактирование
        ...
        <div className="chat-input__row">
          <button className="chat-attach" onClick={() => imageRef.current?.click()} aria-label="Фото">
            <IconImage size={21} />
          </button>
          <button className="chat-attach" onClick={() => fileRef.current?.click()} aria-label="Файл">
          ...
          <textarea
            ...
            placeholder="Сообщение…  [[связь]], #тег, - [ ] дело"
            rows={1}
          />
      ...
      {menuMsg && (
        <Sheet title="Сообщение" onClose={() => setMenuMsg(null)}>
          <div className="stack">
